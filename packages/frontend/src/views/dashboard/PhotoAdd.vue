  ...
  <section class="dashboard-photos-add">
    ...
    >
      <div class="dashboard__side">
        ...
        <!-- Categories -->
        <label v-if="categories" class="dashboard__label">
          ...
          >
            <input v-model="choosedCategories" type="checkbox" :value="cat" />
            {{ cat }}
          </label>
        </label>
        ...
        <div class="dashboard__label">
          ...
          <ul class="dashboard__list-imgs">
            <li v-for="(file, idx) in selectedImages" :key="idx">
              ...
              <label class="dashboard__label">
                <input v-model="file.isPreview" type="checkbox" :value="true" />
      ...
      <div class="dashboard__side dashboard__area-preview">
        <div v-if="selectedImages.length" class="photos">
...
const allPhotoCollections = computed(() => {
  ...
  if (props.isEdit)
    return order.value && +order.value > count ? +order.value + 1 : count;
...
function submit() {
  ...
  if (imgs.length) {
    if (imgs.filter((v) => v.isPreview).length !== 3) {
      clientErrors.value.push("Please choose 3 preview images");
      return;
    }
  } else {
  ...
  if (!order.value && order.value !== 0) {
    clientErrors.value.push("Please select order");
    return;
  }
  if (!title.value) {
    clientErrors.value.push("Please fill title");
    return;
  }
...
function create() {
  try {
    ...
    if (choosedCategories.value)
      formData.append("categories", JSON.stringify(choosedCategories.value));
    for (const photo of selectedImages.value)
      formData.append("photos[]", photo.file as File);
    formData.append(
      "photosInfo",
      JSON.stringify(
...
function update() {
  try {
    const formData = new FormData();
    const oldPhotos: PhotoDraft[] = props.photoCollection?.photos
      ? Array.from(props.photoCollection.photos)
      : [];

    const newPhotoInfo = selectedImages.value
      .filter((v) => {
        if (v.file) {
          formData.append("photos[]", v.file as File);
          return true;
        }
        return false;
      })
    ...
    const existingPhotoInfo = oldPhotos.filter(
      (exphoto) => !updatePhotoInfo.some((up) => up.id === exphoto.id),
    );
