  ...
  <section class="dashboard-works-add">
    ...
    >
      <div class="dashboard__side">
        ...
        <div class="dashboard__label">
          <!-- files: edit -->
          <ul v-if="isEdit" class="dashboard__list-imgs">
          ...
          <!-- files: add -->
          <ul v-else class="dashboard__list-imgs">
            <li v-for="(file, idx) in selectedImages" :key="idx">
              <span class="dashboard__badge badge-yellow">{{ idx + 1 }}</span>
              ...
              <p
                :class="[
                  ...
                  {
                    oversize:
                      (file.file?.size ?? 0) / 1024 >= allowedImageSizeInKb,
                  },
                ]"
              >
              ...
              <label class="dashboard__label">
                ...
                <select v-model="file.order">
                  ...
                  <template v-if="isEdit">
                    ...
                    >
                      {{
                        [...selectedImages, ...(work?.photos ?? [])].length -
                        index
                      }}
                    </option>
                  </template>
        ...
        <label class="dashboard__label">
          ...
          <select v-if="worksLength.length" v-model="order">
            <option disabled selected value="null">Please choose order</option>
            <option
              v-for="(w, index) in worksLength"
              :key="index"
              :value="index"
            >
              {{ index }}
      ...
      <div class="dashboard__side dashboard__area-preview">
        <div
          v-if="previewWork && previewWork.photos && previewWork.photos.length"
          class="dashboard-works-add__preview-cont"
        >
          <WorkView :is-preview="true" :preview-work="previewWork" />
        </div>
      </div>
    </form>
  </section>
...
async function submit() {
  ...
  if (imgs.length) {
    if (!imgs.some((v) => v.isPreview)) {
      clientErrors.value.push("Please choose preview image for the work");
      return;
    }
  } else {
...
function update() {
  ...
  const deletedPhotoIds =
    (WORK.deletedPhotoIds as unknown[] | undefined)?.map(
      (id) => id || id === 0,
    ) ?? [];
  ...
  const existingPhotoInfo = oldPhotos.filter(
    (ex) => !updatePhotoInfo.some((up) => up.id === ex.id),
  );
