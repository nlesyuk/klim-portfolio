...
function installVimeo() {
  const v =
    typeof props.source.videos === "string"
...
function playVideo() {
  if (!player.value) {
    Promise.resolve(installVimeo()).then(() => {
      player.value?.play();
      isPlaying.value = true;
    });
    return;
  }
  player.value.setCurrentTime(0).then(() => {
    player.value?.play();
    isPlaying.value = true;
  });
}
...
function goTo() {
  ...
  if (Object.prototype.hasOwnProperty.call(s, "photoId") && s.photoId)
    router.push({ path: `/photo/${s.photoId}` });
  else if (Object.prototype.hasOwnProperty.call(s, "workId") && s.workId)
    router.push({ path: `/work/${s.workId}` });
}
...
watch(
  ...
  (id) => {
    ...
    if (id === props.slideId) playVideo();
    else pauseVideo();
  },
);

onMounted(() => {
  if (props.source.type === "video") playVideo();
});
</script>
