<template>
  <figure :class="[classes, 'vimeo']">
    ...
    <div ref="videoEl" :class="['vimeoPlayer', 'video-responsive', { 'vimeoPlayer-hidden': !isPlaying }]"></div>
  </figure>
</template>
...
function installVimeo() {
  ...
  player.value = new Player(videoEl.value, { id: +props.id || 521769877, width: 640, color: "#ff0000" });
  player.value.on("ended", () => { isPlaying.value = false; });
}

function playVideo() {
  if (!player.value) installVimeo();
  if (player.value) { player.value.play(); isPlaying.value = true; }
}
</script>
