...
const config = [
  {
    ...
    footerDescription: "Klim Stepan. Ukraine based cinematogapher / director of photography",
    theme: "dark"
  },
  {
...
export const author: string | undefined = config.find(v => v.domain === domain)?.author;
...
export const menuSinematographer = mainMenu.filter(v => v.isSinematographer).map(v => v.name);
export const menuPhotographer = mainMenu.filter(v => v.isPhotographer).map(v => v.name);
export const menu = isCinematographerMode ? menuSinematographer : menuPhotographer;

// Stepan
export const cinematographerCategories = ["all", "film", "automotive", "fashion", "lifestyle", "personal", "commerce"];
// Anna
export const photographerCategories = ["all", "automotive", "fashion", "architecture and interior", "personal"];

export const categories = isCinematographerMode ? cinematographerCategories : photographerCategories;
